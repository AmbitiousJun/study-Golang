...
>
> append函数的参数接收的值必须是切片，所以传入一个数组的时候会报错。
...
在**函数**章节中已经给出使用指针作为函数参数，交换两个整型变量值的例子，这里不在举例。
...
数组和切片的区别是数组的长度不可改变，而切片的长度是可以改变的，即动态数组。
...
>  这种方式在之前小记中提到过，并与java动态初始化数组进行比较。
...
>    1. 原切片容量如果超过`1024`：每次增大的容量为原切片容量的`1/4`，当切片容量大于等于当前所需容量时，停止扩容
...
在这个例子中，结构体就相当于面向对象语言中的实现类，结构体实现接口的标志就是实现接口中定义的**所有方法**。
...
- 当接收端从通道中接收数据时，如果通道中没有数据，通道也会立即阻塞，直到发送端将数据写入时，阻塞才会解除并接收数据。
...
- 线程数判断：程序中没有启动子线程，也就是说当前程序只有一个主线程。
- 逻辑判断：通道的工作机制是，当通道中没有数据时，接收方想要接收数据就会阻塞接收方线程，直到通道中进来一个新的数据，才能够解除阻塞并接收数据。
